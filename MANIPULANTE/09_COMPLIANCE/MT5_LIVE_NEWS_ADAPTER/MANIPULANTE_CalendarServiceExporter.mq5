//+------------------------------------------------------------------+
//|                                MANIPULANTE_CalendarServiceExporter|
...
//--- input parameters
input int      RefreshIntervalSeconds = 3600; // Refrescar cada hora
...
void OnStart()
{
   Print("MANIPULANTE SERVICE: Iniciando Exportador de Calendario...");
   
   while(!IsStopped())
   {
      ExportAll();
      
      // Dormir hasta el próximo intervalo o hasta que se detenga el servicio
      for(int i=0; i<RefreshIntervalSeconds && !IsStopped(); i++)
         Sleep(1000);
   }
   
   Print("MANIPULANTE SERVICE: Detenido.");
}
...
void ExportToFiles(datetime start_time, datetime end_time, string label)
{
   ...
   if(count < 0)
   {
      Print("MANIPULANTE SERVICE: Error al obtener calendario. Código: ", GetLastError());
      return;
   }
   ...
   FileWriteString(handle_json, "{\n  \"source\": \"MT5_MQL5_CALENDAR_SERVICE\",\n  \"generated_at_utc\": \"" + generated_at + "\",\n  \"events\": [\n");
   ...
   for(int i=0; i<count; i++)
   {
      ...
      // Filtros EUR/USD basados en la moneda del país
      ...
      // Escribir CSV
      FileWrite(handle_csv, event.name, country.currency, country.code, imp_str, t_str, "MT5_MQL5_CALENDAR_SERVICE", generated_at);
   ...
   Print("MANIPULANTE SERVICE: Exportados ", exported_count, " eventos para ", label);
}
