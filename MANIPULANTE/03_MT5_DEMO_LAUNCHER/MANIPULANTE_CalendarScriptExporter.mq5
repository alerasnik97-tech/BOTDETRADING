//+------------------------------------------------------------------+
//|                                MANIPULANTE_CalendarScriptExporter|
...
//--- input parameters
input string   ExportFileNameSuffix = "mql5_calendar";
...
void OnStart()
{
   Print("MANIPULANTE SCRIPT: Iniciando Exportación de Calendario (Bootstrap)...");
   ExportAll();
   Print("MANIPULANTE SCRIPT: Exportación finalizada.");
}
...
void ExportToFiles(datetime start_time, datetime end_time, string label)
{
   ...
   if(count < 0)
   {
      Print("MANIPULANTE SCRIPT: Error al obtener calendario. Código: ", GetLastError());
      return;
   }
   ...
   FileWriteString(handle_json, "{\n  \"source\": \"MT5_MQL5_CALENDAR_BOOTSTRAP\",\n  \"generated_at_utc\": \"" + generated_at + "\",\n  \"events\": [\n");
   ...
   for(int i=0; i<count; i++)
   {
      ...
      FileWrite(handle_csv, event.name, country.currency, country.code, imp_str, t_str, "MT5_MQL5_CALENDAR_BOOTSTRAP", generated_at);
   ...
   Print("MANIPULANTE SCRIPT: Exportados ", exported_count, " eventos para ", label);
}
