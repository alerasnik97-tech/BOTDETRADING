...
PHASE_OUT = LAB / "reports" / "v49_7b_controlled_restart_memory_safe"
...
def run_controlled_v49_7b(target_n=800, mode="full"):
    PHASE_OUT.mkdir(parents=True, exist_ok=True)
    
    # Load or reuse configs
    ...
    df_cfg.to_csv(PHASE_OUT / "R1_V49_7B_CONTROLLED_CONFIGS.csv", index=False)
    ...
    log_file = PHASE_OUT / "R1_V49_7B_CONTROLLED_RUN_LOG.txt"
    trades_file = PHASE_OUT / "R1_V49_7B_CONTROLLED_TRADES.csv"
    
    # Initialize trades file with header
    ...
    with open(log_file, "a") as f_log:
        f_log.write(f"Starting Controlled {mode} Run at {datetime.now()}\n")
        f_log.flush()
        ...
        for y, m in months:
            try:
                ...
                for b_idx in range(0, len(configs), batch_size):
                    ...
                    # Flush batch trades to CSV
                    if batch_trades:
                        with open(trades_file, "a", newline="") as f_tr:
                            writer = csv.DictWriter(f_tr, fieldnames=["config_id", "phase", "entry_time", "exit_time", "entry_price", "exit_price", "direction", "pnl_net_r", "slippage"])
                            writer.writerows(batch_trades)
                    
                    f_log.write(f"  Batch {b_idx//batch_size + 1} DONE ({len(batch_trades)} trades).\n")
                    f_log.flush()
        ...
        f_log.write(f"Completed Controlled {mode} Run at {datetime.now()}\n")
        f_log.flush()

    # Final Ranking (only for full mode)
    if mode == "full" and trades_file.exists():
        ...
        pd.DataFrame(summary).sort_values("PF_val", ascending=False).to_csv(PHASE_OUT / "R1_V49_7B_CONTROLLED_CANDIDATE_RANKING.csv", index=False)

if __name__ == "__main__":
    mode = sys.argv[1] if len(sys.argv) > 1 else "preflight"
    run_controlled_v49_7b(mode=mode)
