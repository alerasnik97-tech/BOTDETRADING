...
RESULTS_DIR = Path("results") / "ict_atomic_setups"
PAIR = "EURUSD"
TIMEFRAME = "M5"
STRATEGY_MODULES = [
    ict_atomic_sweep_displacement_pm,
...
SERIOUS_DEV_MIN_TRADES_PER_MONTH = 0.45
SERIOUS_DEV_MIN_PF = 1.05
SERIOUS_DEV_MIN_EXPECTANCY = 0.02
SERIOUS_MIN_VAL_HOLD_TRADES = 5
SERIOUS_MIN_VAL_HOLD_PF = 0.95


def build_output_root() -> Path:
    timestamp = pd.Timestamp.now(tz=NY_TZ).strftime("%Y%m%d_%H%M%S")
    root = RESULTS_DIR / f"{timestamp}_ict_atomic_setups"
...
def selection_score(summary: dict[str, Any]) -> float:
    ...
    score = 0.0
    score += pf_for_score * 120.0
    score += float(summary["expectancy_r"]) * 900.0
    score += float(summary["total_return_pct"]) * 1.2
    score -= float(summary["max_drawdown_pct"]) * 3.0
    score -= float(summary["negative_years"]) * 25.0
    score -= float(summary["news_exit_count"]) * 4.0
    ...
    if total_trades < SERIOUS_DEV_MIN_TRADES:
        score -= 260.0
    elif total_trades < SERIOUS_DEV_MIN_TRADES + 8:
        score -= 90.0
...
def verdict_from_selected(selected_row: pd.Series, serious_candidates: int) -> str:
    ...
    if bool(selected_row["serious_candidate"]):
...
def main() -> None:
    ...
    news_result = require_operational_news(PAIR, news_config, context="ict_atomic_runner")
    ...
    for strategy_module in STRATEGY_MODULES:
        ...
        export_strategy_bundle(
            ...
            extra_json={
                ...
                "period_summaries": {label: bundle_["summary"] for label, bundle_ in period_bundles.items()},
            },
        )
    ...
    strategy_scorecard.to_csv(output_root / "ict_strategy_scorecard.csv", index=False)
    manifest = {
        "pair": PAIR,
        "timeframe": TIMEFRAME,
