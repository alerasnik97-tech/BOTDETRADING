...
class BO01ContractTests(unittest.TestCase):
    ...
    def test_short_side_eligible_signal_contract(self) -> None:
        module = _module()
        frame, i = _bo01_frame(direction="short")
        result = module.signal(frame, i, module.default_params())
        self.assertIsNotNone(result)
        self.assertEqual(result["signal"], -1)
        self.assertEqual(result["direction"], "short")
        self.assertEqual(result["target_rr"], 2.0)
        self.assertGreater(result["stop_price"], float(frame["close"].iat[i]))
        _assert_signal_contract(self, result)
