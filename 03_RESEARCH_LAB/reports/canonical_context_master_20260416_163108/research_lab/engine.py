...
def run_backtest(
    ...
    *,
    ...
    from research_lab.news_filter import build_entry_block_details
    if news_events is not None and news_settings is not None and not news_events.empty:
        news_details = build_entry_block_details(bar_open_local, news_events, news_settings)
    else:
    ...
    for i in range(strategy_module.WARMUP_BARS, len(frame)):
        ...
        if pending_signal is not None and getattr(engine_config, "cancel_pending_pre_news", True):
            if news_block[i]:
        ...
        if position is not None:
            ...
            # PRIORIDAD 4: NEWS FORTRESS - FORCED EXIT (HARD KILL)
            if exit_reason is None and getattr(engine_config, "forced_exit_pre_news", True):
                if news_block[i]:
                    # Hemos entrado en la zona de peligro de noticias (Kill Zone)
            ...
            if exit_reason is not None and exit_signal_price is not None:
                ...
                blocking_rule_used = ""
                if blocked_by_news:
                    blocking_event_name = str(news_details["blocking_event_name"].iat[i])
                    blocking_event_time_ny = str(news_details["blocking_event_time_ny"].iat[i])
                    blocking_rule_used = str(news_details["blocking_rule_used"].iat[i])
                if precision_enabled:
        ...
        if position is None and pending_signal is None and i < len(frame) - 1:
            if i <= cooldown_until_index or not np.isfinite(range_atr[i]) or range_atr[i] > engine_config.shock_candle_atr_max:
                ...
                if signal is not None:
                    signal = validate_signal_risk_contract(signal, signal_price=float(close[i]), engine_config=engine_config)
