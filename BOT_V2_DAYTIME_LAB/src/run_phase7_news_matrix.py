...
def run_news_matrix():
    print("Starting Phase 7 News Refinement (Fase 5)...")
    ...
    guards = [0, 30, 60] # News Block Minutes
    results = []
    output_dir = Path(r"C:\Users\alera\Desktop\Bot\BOT DE TRADING ultimo\BOT_V2_DAYTIME_LAB\outputs\phase7_m3_choch_refinement\news_refinement")
    ...
    for g in guards:
        ...
        if not full_trades.empty:
            ...
            results.append({'guard': g, 'sample': len(full_trades), 'pf': round(pf, 2)})
            print(f"  -> PF={pf:.2f} Sample={len(full_trades)}")

    # Special Test: Post-News Only
    # This requires a slightly different logic in the engine, but we can simulate it 
    # by taking the trades and filtering them if there was no news today.
    # Actually, I'll just run one more with a flag in the future, but for now let's just do guards.
    
    pd.DataFrame(results).to_csv(output_dir / "news_matrix_results.csv", index=False)
    print("News Matrix Complete.")
