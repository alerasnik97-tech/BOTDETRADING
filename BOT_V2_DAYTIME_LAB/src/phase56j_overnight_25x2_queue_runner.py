...
def process_month(batch_id, m_str, batch_dir):
    ...
    for _, trade in df_month.iterrows():
        ...
        results.append({
            ...
            "entry_price": entry_price, "sl": sl, "tp": tp, "risk_pips": round(risk_pips, 2),
            "outcome": outcome, "pnl_base": round(pnl_base, 4) if not np.isnan(pnl_base) else "NaN", 
            "pnl_net_ftmo": round(pnl_net_ftmo, 4) if not np.isnan(pnl_net_ftmo) else "NaN",
            "commission_r": round(commission_r, 4),
            "auditable_yes_no": auditable_yes_no,
            "non_auditable_reason": non_auditable_reason
        })

    res_df = pd.DataFrame(results)
    # Convert NaN strings back to actual NaNs for calculation
    res_df["pnl_base"] = pd.to_numeric(res_df["pnl_base"], errors='coerce')
    res_df["pnl_net_ftmo"] = pd.to_numeric(res_df["pnl_net_ftmo"], errors='coerce')
    ...
    total_sample = len(res_df)
    auditables = res_df[res_df["auditable_yes_no"] == "YES"].copy()
    non_auditables_count = total_sample - len(auditables)
    no_auditable_pct = (non_auditables_count / total_sample) if total_sample > 0 else 0

    if auditables.empty:
        verdict = "MONTH_BLOCKED_NO_AUDITABLE_TRADES"
        total_r_base, total_r_net, pf_base, pf_net = 0, 0, 0, 0
    else:
        total_r_base = auditables["pnl_base"].sum()
        total_r_net = auditables["pnl_net_ftmo"].sum()
        
        pos_base = auditables[auditables["pnl_base"] > 0]["pnl_base"].sum()
        neg_base = abs(auditables[auditables["pnl_base"] < 0]["pnl_base"].sum())
        pf_base = pos_base / neg_base if neg_base > 0 else 9.99
        
        pos_net = auditables[auditables["pnl_net_ftmo"] > 0]["pnl_net_ftmo"].sum()
        neg_net = abs(auditables[auditables["pnl_net_ftmo"] < 0]["pnl_net_ftmo"].sum())
        pf_net = pos_net / neg_net if neg_net > 0 else 9.99
        
        if no_auditable_pct > 0.20:
            verdict = "INCONCLUSIVE_WITH_WARNINGS"
        else:
            verdict = "NET_FTMO_FAILS"
            if pf_net > 1.5 and (total_r_net / len(auditables)) > 0.1: verdict = "NET_FTMO_STRONG"
            elif pf_net > 1.2 and total_r_net > 0: verdict = "NET_FTMO_SURVIVES"
            elif pf_net > 1.05 and total_r_net > 0: verdict = "NET_FTMO_FRAGILE"

    metrics = {
        "month": m_str, "sample_total": total_sample, "auditables": len(auditables), 
        "non_auditables": non_auditables_count,
        "total_R_base": round(float(total_r_base), 2), "PF_base": round(float(pf_base), 2),
        "total_R_net_FTMO": round(float(total_r_net), 2), "PF_net_FTMO": round(float(pf_net), 2),
    ...
    # Data Quality Report
    dq = {
        "month": m_str, "parquet_exists": os.path.exists(parquet_path),
        "sample_total": total_sample, "auditables": len(auditables),
        "non_auditables": non_auditables_count,
        "non_auditable_pct": round(no_auditable_pct * 100, 2),
        "reasons": res_df[res_df["auditable_yes_no"] == "NO"]["non_auditable_reason"].value_counts().to_dict()
    }
    with open(os.path.join(batch_dir, f"PHASE56_MONTH_{m_clean}_DATA_QUALITY.json"), "w") as f:
    ...
    t_samp = sum(p.get("sample", p.get("sample_total", 0)) for p in all_canonic)
    cp["summary"] = {
