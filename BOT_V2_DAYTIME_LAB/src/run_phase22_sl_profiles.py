...
def run_sl_profiles():
    print("Fase 3: Testing SL Profiles on Top Variant...")
    ...
    sl_offsets = [0.0, 0.5, 1.0, 1.5, 2.0] # Pips
    results = []
    
    for offset in sl_offsets:
        label = f"SL_Peak_plus_{offset}"
        choch_detector = First3MChochDetector({'max_mins_post_sweep': 60, 'sl_buffer': offset})
    ...
    pd.DataFrame(results).to_csv(r"C:\Users\alera\Desktop\Bot\BOT DE TRADING ultimo\BOT_V2_DAYTIME_LAB\outputs\phase22_high_wr_low_dd\sl_profiles\sl_profile_results.csv", index=False)
