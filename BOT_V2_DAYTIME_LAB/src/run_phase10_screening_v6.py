...
class Phase10DiscoveryEngineV6:
    def __init__(self):
        self.tz_ny = 'America/New_York'

    def run_screening(self, df_m5, df_h1):
        trades = []
        df_m5 = df_m5.copy()
        ...
        while i < total:
            ...
            if lvl:
                ...
                # FAKEOUT + BIAS + DISTANCE
                if prev.high > lvl['high'] and row.close < lvl['high'] and d > 20 and row.ema_slope < 0:
                    setup = {'direction': 'SHORT', 'entry_p': row.close, 'sl': prev.high + 0.0001, 'tp': row.close - (prev.high - row.close) * 2.0}
                    res, _ = self.resolve(df_m5, i, setup)
                    if res is not None: trades.append({'time': curr_time, 'result': res}); i += 20; continue
                elif prev.low < lvl['low'] and row.close > lvl['low'] and d < -20 and row.ema_slope > 0:
    ...
    def resolve(self, df, idx, setup):
        ...
        return 0.0, df.iloc[min(idx + 200, len(df)-1)].timestamp_ny

def run_test():
    print("Phase 10: Discovery Screening V6 (Fakeout + Bias + Dist)")
    engine = Phase10DiscoveryEngineV6()
    ...
    trades = engine.run_screening(df_m5, df_h1)
    if not trades.empty:
