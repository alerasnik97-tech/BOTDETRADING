
import os
import zipfile
from pathlib import Path

def create_and_validate_zip():
    root = Path(r"C:\Users\alera\Desktop\Bot\BOT DE TRADING ultimo")
    zip_path = root / "000_PARA_CHATGPT.zip"
    
    include_dirs = ['BOT_V2_DAYTIME_LAB', 'SCBI_M5_GLOBAL', 'docs', 'configs']
    include_subdirs = {'phase23_consistency_repair', 'phase24_controlled_optimization_2015_2026', 'phase25_max_robust_plateau'}
    
    exclude_parts = {'.git', '.venv', '__pycache__', 'raw_data', 'data', 'results', 'research_lab', 'ARCHIVE_SUPERSEDED', '.ipynb_checkpoints'}
    exclude_exts = {'.zip', '.csv', '.parquet', '.log', '.png', '.jpg', '.pdf', '.exe', '.dll', '.bin'}
    
    print(f"Creating REPAIRED ZIP at {zip_path}...")
    
    try:
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
            for f in root.iterdir():
                if f.is_file() and f.suffix == '.md': zf.write(f, f.name)
            
            for inc_dir in include_dirs:
                dir_path = root / inc_dir
                if not dir_path.exists(): continue
                for file_path in dir_path.rglob('*'):
                    if any(part in exclude_parts for part in file_path.parts): continue
                    
                    is_summary = ('phase24' in str(file_path) or 'phase25' in str(file_path)) and file_path.suffix.lower() == '.csv'
                    
                    if file_path.suffix.lower() in exclude_exts and not is_summary:
                        if file_path.suffix.lower() == '.csv' and file_path.stat().st_size < 100000: pass
                        else: continue
                    
                    if file_path.is_file():
                        if file_path.name in {'.env', 'secrets', 'credentials'}: continue
                        zf.write(file_path, file_path.relative_to(root))
                        
        print("REPAIRED ZIP Created. Validating...")
        with zipfile.ZipFile(zip_path, 'r') as zf:
            test_res = zf.testzip()
            namelist = zf.namelist()
            print(f"Total files in ZIP: {len(namelist)}")
            critical = ["BOT_V2_DAYTIME_LAB/configs/phase25_forward_demo_candidate_config.json", "01_CURRENT_PROJECT_STATUS.md"]
            for c in critical:
                if c not in namelist: print(f"ERROR: MISSING {c}")
                
    except Exception as e: print(f"PROCESS ERROR: {e}")

if __name__ == "__main__":
    create_and_validate_zip()
