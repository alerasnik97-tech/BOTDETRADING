...
def run_quality_filters():
    print("Fase 5: Quality Filters for High Winrate Optimization...")
    ...
    for b_pct in body_filters:
        label = f"Body_{int(b_pct*100)}"
        sig_f = signals_all[signals_all['body_pct'] >= b_pct].copy()
        sig_f['hour'] = sig_f['choch_time'].dt.hour
        sig_f = sig_f[(sig_f['hour'] >= 7) & (sig_f['hour'] < 16.5)]
        sig_f['date'] = sig_f['choch_time'].dt.date
        sig_f = sig_f.sort_values('choch_time').groupby('date').head(1)
        ...
        results.append({"config": label, "pf": pf, "winrate": wr, "sample": len(t_s)})
        print(f"{label}: PF {pf} | WR {wr} | Sample {len(t_s)}")
        
    pd.DataFrame(results).to_csv(r"C:\Users\alera\Desktop\Bot\BOT DE TRADING ultimo\BOT_V2_DAYTIME_LAB\outputs\phase22_high_wr_low_dd\quality_filters\quality_filter_results.csv", index=False)
