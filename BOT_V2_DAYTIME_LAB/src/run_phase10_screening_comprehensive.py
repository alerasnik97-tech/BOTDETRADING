...
class Phase10DiscoveryEngine:
    ...
    def run_family_screening(self, df_m5, df_h1, news_df, family_id, config):
        ...
        # Fractals for Family 2 (Fast CHoCH N=3)
        if family_id == 2:
            df_m5['is_h_frac'], df_m5['is_l_frac'] = self.get_fractals(df_m5, n=3)
        ...
        while i < len(df_m5):
            ...
            # FAMILY 2: H1 Sweep + Fast CHoCH
            elif family_id == 2:
                lvl = levels.get(curr_date)
                if lvl:
                    if row.high > lvl['high'] or row.low < lvl['low']:
                        # Look for CHoCH in next few bars
                        for k in range(i, min(i+12, len(df_m5))):
                            f_row = df_m5.iloc[k]
                            if row.high > lvl['high'] and f_row.is_h_frac: # Sweep H, find high fractal
                                pass # This logic needs stateful CHoCH detection, skipped for speed in screening
            ...
            # FAMILY 3: Displacement Candle (M5)
                ...
                if lvl:
                    if row.high > lvl['high'] or row.low < lvl['low']:
                        # Candle Body >= 50% and Displacement
                        body = abs(row.close - row.open)
                        rng = row.high - row.low
                        if rng > 0 and body / rng >= 0.6:
                            if row.high > lvl['high'] and row.close < row.open: # Bearish Displacement after Sweep H
                                setup = {'direction': 'SHORT', 'entry_p': row.close, 'sl': row.high + 0.0001, 'tp': row.close - (row.high - row.close + 0.0001) * 1.5}
                            elif row.low < lvl['low'] and row.close > row.open: # Bullish Displacement after Sweep L
                                setup = {'direction': 'LONG', 'entry_p': row.close, 'sl': row.low - 0.0001, 'tp': row.close + (row.close - row.low + 0.0001) * 1.5}
            ...
            if setup:
                ...
                if not in_news:
                    ...
                    if res is not None:
                        ...
                        i += 1; continue # Don't skip for screening to find all potential setups
    ...
    def resolve_trade(self, df, start_idx, setup, config):
        ...
        for j in range(start_idx + 1, min(start_idx + 200, len(df))):
            ...
            if (f.timestamp_ny - setup.get('time', f.timestamp_ny)).total_seconds() > 8*3600: return 0.0, f.timestamp_ny
        return None, None

def run_screening():
    print("Phase 10: Discovery Screening - Comprehensive")
