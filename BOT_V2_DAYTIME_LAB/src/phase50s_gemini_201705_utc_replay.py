...
parquet_path = r"C:\Users\alera\Desktop\Bot\BOT DE TRADING ultimo\BOT_MARKET_DATA\tick\EURUSD\monthly\EURUSD_ticks_2017_05.parquet"
...
df_trades = df_trades[df_trades['year_month'].astype(str) == '2017-05'].copy()
...
if not os.path.exists(parquet_path):
    ...
    # If missing, we still want to save metrics as NO_AUDITABLE
    df_results_placeholder = pd.DataFrame()
else:
...
for idx, row in df_trades.iterrows():
    ...
    results.append({
        ...
        'first_sl_time_utc': str(first_sl_time_utc) if first_sl_time_utc else None,
        'first_be_trigger_time_utc': str(first_be_trigger_time_utc) if first_be_trigger_time_utc else None,
        'first_be_stop_time_utc': str(first_be_stop_time_utc) if first_be_stop_time_utc else None,
        'first_tp_time_utc': str(first_tp_time_utc) if first_tp_time_utc else None,
...
csv_out_path = os.path.join(output_dir, 'PHASE50S_201705_GEMINI_TRADE_LEVEL.csv')
...
json_out_path = os.path.join(output_dir, 'PHASE50S_201705_GEMINI_METRICS.json')
with open(json_out_path, 'w') as f:
    json.dump(metrics, f, indent=4)

# Data Quality Output
with open(os.path.join(output_dir, 'PHASE50S_201705_DATA_QUALITY.json'), 'w') as f:
    json.dump(dq, f, indent=4)

# Task 5: TIME_EXIT stress if applies
if auditables > 0 and (time_exit_count / auditables > 0.3):
    ...
    with open(os.path.join(output_dir, 'PHASE50S_201705_TIME_EXIT_STRESS.json'), 'w') as f:
        json.dump(stress_models, f, indent=4)

print(f"Done. Processed {sample_total} trades.")
