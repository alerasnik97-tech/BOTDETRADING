...
def run_full_screening():
    print("Starting Phase 13 Full Screening...")
    ...
    # --- Method 1: Sweep + Momentum ---
    print("Screening Method 1 (Sweep + Momentum)...")
    for win_start, win_end in windows:
        for body_pct in [0.5, 0.6, 0.7]:
            p = {
                "momentum_body_pct": body_pct, 
                "momentum_relative_size": 1.2, 
                "max_bars_post_sweep": 6, 
                "use_bias": True, 
                "bias_map": bias_map_h1,
                "min_sweep_depth_pips": 1.5,
                "sl_buffer_pips": 1.0
            }
            config = {
                "method": "sweep", "params": p, "tp_r": 2.5, "sl_pips_plus": 0.5,
                "news_guard_mins": 30, "start_time": win_start, "end_time": win_end,
            ...
            metrics['method'] = 'sweep'
            metrics['variant'] = f"sweep_{win_start}_{body_pct}"
            results.append(metrics)
            print(f"  {metrics['variant']}: PF={metrics['pf']}, Sample={metrics['sample']}")

    # --- Method 2: Reclaim ---
    print("Screening Method 2 (Reclaim)...")
    for win_start, win_end in windows:
        for body_pct in [0.5, 0.6, 0.7]:
            p = {
                "reclaim_body_pct": body_pct, 
                "session_type": "london", 
                "use_bias": True, 
                "bias_map": bias_map_h1
            }
            config = {
                "method": "reclaim", "params": p, "tp_r": 2.5, "sl_pips_plus": 1.0,
                "news_guard_mins": 30, "start_time": win_start, "end_time": win_end,
                "one_trade_per_day": True, "use_m15_bias": True, "m15_bias_map": bias_map_m15
            }
            trades = engine.run_backtest(df_m3, levels, news_df, config)
            ...
            metrics['variant'] = f"reclaim_{win_start}_{body_pct}"
            results.append(metrics)
            print(f"  {metrics['variant']}: PF={metrics['pf']}, Sample={metrics['sample']}")

    # Save results
    df_results = pd.DataFrame(results)
    df_results.to_csv(output_base / "phase13_full_screening_results.csv", index=False)
    
    print("Full Screening Complete.")
