...
def run_sample_audit():
    print("Fase 2 (Sample 2024): Running forensic audit on 2024 data...")
    ...
    df_ltf = df_ltf[df_ltf['timestamp_ny'].dt.year == 2024].copy().reset_index(drop=True)
    print(f"Sample size (2024): {len(df_ltf)}")
    
    df_news = engine.load_news(period)
    gate = NewsFortressGate(df_news)
    df_h1 = engine.load_and_prep_prices(period, timeframe='h1')
    df_h1 = df_h1[df_h1['timestamp_ny'].dt.year == 2024].copy().reset_index(drop=True)
    
    sweeps_detector = H1FractalSweepDetector({})
    sweeps = sweeps_detector.detect_sweeps(df_h1)
    ...
    for _, sig in signals.iterrows():
        ...
        # Sincronización con Phase14Engine
        try:
        ...
        curr_sl, is_be_active, ambiguous = sl_p, False, False
        for t, bar in future.iterrows():
            if direction == 'LONG':
                trig_be, hit_sl = bar['high_bid'] >= be_trigger, bar['low_bid'] <= curr_sl
                if trig_be and hit_sl and not is_be_active: res, exit_time, exit_price, ambiguous = 'SL', t, curr_sl, True; break
                if hit_sl: res, exit_time, exit_price = ('SL' if not is_be_active else 'BE'), t, curr_sl; break
                if bar['high_bid'] >= tp_price: res, exit_time, exit_price = 'TP', t, tp_price; break
                if trig_be: is_be_active, curr_sl = True, entry_price
            else:
                trig_be, hit_sl = bar['low_bid'] <= be_trigger, bar['high_ask'] >= curr_sl
                if trig_be and hit_sl and not is_be_active: res, exit_time, exit_price, ambiguous = 'SL', t, curr_sl, True; break
                if hit_sl: res, exit_time, exit_price = ('SL' if not is_be_active else 'BE'), t, curr_sl; break
                if bar['low_bid'] <= tp_price: res, exit_time, exit_price = 'TP', t, tp_price; break
                if trig_be: is_be_active, curr_sl = True, entry_price
    ...
    df_res = pd.DataFrame(audit_trades)
    print(f"2024 Audit complete. trades: {len(df_res)}")
    ...
    df_res.to_csv(out_dir / "phase22_be_05_audit_full.csv", index=False)
