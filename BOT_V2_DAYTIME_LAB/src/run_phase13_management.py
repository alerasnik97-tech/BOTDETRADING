...
def run_management():
    print("Starting Phase 13 Management Matrix...")
    ...
    results = []
    output_base = Path(r"C:\Users\alera\Desktop\Bot\BOT DE TRADING ultimo\BOT_V2_DAYTIME_LAB\outputs\phase13_two_practical_entries\management")
    output_base.mkdir(parents=True, exist_ok=True)
    
    # Management Matrix for Method 2 Top (London Reclaim)
    tps = [2.0, 2.5, 3.0, 3.5]
    bes = [None, 1.0, 1.5]
    
    print("Evaluating Management Matrix for London Reclaim...")
    for tp in tps:
        for be in bes:
            ...
            metrics['variant'] = f"tp_{tp}_be_{be}"
            results.append(metrics)
            print(f"  {metrics['variant']}: PF={metrics['pf']}, Sample={metrics['sample']}")

    # Save results
    df_results = pd.DataFrame(results)
    df_results.to_csv(output_base / "method2_management_matrix.csv", index=False)
    
    print("Management Matrix Complete.")
