...
parquet_path = r"C:\Users\alera\Desktop\Bot\BOT DE TRADING ultimo\BOT_MARKET_DATA\tick\EURUSD\monthly\EURUSD_ticks_2017_08.parquet"
...
df_trades = df_trades[df_trades['year_month'].astype(str) == '2017-08'].copy()
...
for idx, row in df_trades.iterrows():
    ...
    # Use initial SL calculated from risk
    if direction == 'LONG':
    ...
    for t_idx, t_row in trade_ticks.iterrows():
        # Evaluate SL/TP/BE on Bid for LONG, Ask for SHORT
        ...
        if not be_triggered:
            ...
            if hits > 1:
                ...
                # Logic: Check if it's the SAME tick.
...
csv_out_path = os.path.join(output_dir, 'PHASE50S_201708_GEMINI_TRADE_LEVEL.csv')
...
json_out_path = os.path.join(output_dir, 'PHASE50S_201708_GEMINI_METRICS.json')
with open(json_out_path, 'w') as f:
    json.dump(metrics, f, indent=4)

print(f"Done. Processed {sample_total} trades.")
print(f"Auditables: {auditables}")
print(f"Total R: {total_R_real:.2f}")
