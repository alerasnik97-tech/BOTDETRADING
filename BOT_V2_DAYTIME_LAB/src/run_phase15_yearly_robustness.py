...
def run_yearly_robustness():
    print("Starting Phase 15 Yearly Robustness Audit...")
    ...
    for year in range(2020, 2026):
        t_year = trades[trades['year'] == year]
        metrics = engine.calculate_metrics(t_year, config)
        metrics['year'] = year
        yearly.append(metrics)
        print(f"Year {year} | PF: {metrics['pf']} | Sample: {metrics['sample']}")
    ...
    pd.DataFrame(yearly).to_csv(out_dir / "phase15_robustness_by_year.csv", index=False)
    print("Yearly Audit Complete.")

if __name__ == "__main__":
    run_yearly_robustness()
