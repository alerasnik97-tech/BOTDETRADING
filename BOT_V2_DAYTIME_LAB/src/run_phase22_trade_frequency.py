...
def run_trade_frequency():
    print("Fase 4: Trade Frequency (1-4 Trades/Day)...")
    ...
    for max_t in [1, 2, 3]:
        ...
        sig_f = signals_all.sort_values('choch_time').groupby('date').head(max_t)
        ...
        results.append({"max_trades": max_t, "pf": pf, "winrate": wr, "sample": len(t_s)})
        print(f"{label}: PF {pf} | WR {wr} | Sample {len(t_s)}")
        
    pd.DataFrame(results).to_csv(r"C:\Users\alera\Desktop\Bot\BOT DE TRADING ultimo\BOT_V2_DAYTIME_LAB\outputs\phase22_high_wr_low_dd\trade_frequency\trade_frequency_1_2_3_results.csv", index=False)
