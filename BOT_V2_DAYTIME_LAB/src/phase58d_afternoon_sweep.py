...
def full_afternoon_sweep():
    ...
    # Range 12:00 - 19:00
    df_afternoon = df[(df['entry_ny'].dt.hour >= 12) & (df['entry_ny'].dt.hour < 19)].copy()
    ...
    for h in range(start_hour, end_hour):
        for m in [0, 30]:
            t_start = time(h, m)
            t_end = time(h, m+29, 59) if m == 0 else time(h, 59, 59)
            
            mask = (df_afternoon['entry_ny'].dt.time >= t_start) & (df_afternoon['entry_ny'].dt.time <= t_end)
            b_trades = df_afternoon[mask]
            
            bin_label = f"{h:02d}:{m:02d} - {h:02d}:{m+29 if m==0 else 59:02d}"
            
            if b_trades.empty:
                results.append({"bin": bin_label, "count": 0, "net_r": 0, "wr": 0, "pf": 0})
                continue
