...
# Paths
TICK_PARQUET = r"C:\Users\alera\Desktop\Bot\BOT DE TRADING ultimo\BOT_MARKET_DATA\tick\EURUSD\monthly\EURUSD_ticks_2025_02.parquet"
...
def audit_202502():
    ...
    df_month = df_raw[df_raw['year_month'] == "2025-02"].copy()
    sample = len(df_month)
    print(f"Trades found: {sample}")
    
    if sample != 19:
        print(f"WARNING: Expected 19 trades, found {sample}")

    print("\n--- TAREA 2: Replay 19:45 NY solo 2025-02 ---")
    ...
    for idx, trade in df_month.iterrows():
        ...
        if idx < df_month.index[3]:
            print(f"Trade {idx}: {direction} entry {entry_time_utc}, exit {exit_time_utc}, ticks: {len(trade_ticks)}")
        ...
        if not trade_ticks.empty:
            ...
            if final_outcome == "TIME_EXIT":
                # Use the last available tick in the window
            ...
            final_r = 0.0
            print(f"WARNING: No ticks for trade {idx} in window {entry_time_utc} to {exit_time_utc}")
    ...
    df_replay.to_csv(os.path.join(REPORT_DIR, "PHASE50Y_LITE_202502_TRADE_LEVEL.csv"), index=False)
    ...
    with open(os.path.join(REPORT_DIR, "PHASE50Y_LITE_202502_METRICS.json"), "w") as f:
        json.dump(metrics, f, indent=4)
    print("Metrics generated.")

    print("\n--- TAREA 3: Stress compacto ---")
    if pf > 1.10 and exp > 0:
        ...
        with open(os.path.join(REPORT_DIR, "PHASE50Y_LITE_202502_STRESS.json"), "w") as f:
            json.dump(stress, f, indent=4)
        print("Stress report generated.")
    else:
        print("Stress skipped (Base fails or weak)")

    print("\n--- TAREA 4: Comparar contra bar-level ---")
    # PF bar 0.70; expectancy bar -0.095R; total_R bar -1.80R; outcomes bar TP 3 / BE 10 / SL 6.
    print(f"Tick PF: {pf:.2f} vs Bar PF: 0.70")
    print(f"Tick R: {total_r:.2f} vs Bar R: -1.80")
