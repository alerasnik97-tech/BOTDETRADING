...
def run_london_screening():
    print("Starting Phase 13 London Screening...")
    ...
    win_start, win_end = "03:00", "12:00"
    
    print("Screening Method 2 (London Reclaim)...")
    for tp in [2.0, 2.5, 3.0]:
        for body_pct in [0.5, 0.6, 0.7]:
            ...
            metrics['variant'] = f"london_reclaim_tp_{tp}_body_{body_pct}"
            results.append(metrics)
            print(f"  {metrics['variant']}: PF={metrics['pf']}, Sample={metrics['sample']}")

    # Save results
    df_results = pd.DataFrame(results)
    df_results.to_csv(output_base / "phase13_london_screening_results.csv", index=False)
    
    print("London Screening Complete.")
