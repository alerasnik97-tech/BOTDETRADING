...
def run_robustness():
    print("Fase 9: Yearly Robustness Audit for Top Candidate...")
    ...
    for _, sig in signals.iterrows():
        ...
        trades.append({"time": entry_time, "res": res})
        
    t_df = pd.DataFrame(trades)
    t_df['year'] = t_df['time'].dt.year
    
    yearly = []
    for year, group in t_df.groupby('year'):
        tp_c, sl_c = len(group[group['res'] == 'TP']), len(group[group['res'] == 'SL'])
        pf = round((tp_c * 2.0) / sl_c, 2) if sl_c > 0 else (tp_c * 2.0)
        yearly.append({"year": year, "sample": len(group), "pf": pf})
        
    pd.DataFrame(yearly).to_csv(r"C:\Users\alera\Desktop\Bot\BOT DE TRADING ultimo\BOT_V2_DAYTIME_LAB\outputs\phase20_news_fortress_frequency_recovery\robustness\phase20_robustness_by_year.csv", index=False)
    print(yearly)
