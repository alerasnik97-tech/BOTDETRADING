...
def run_management_test(name, config_patch):
    print(f"  Testing Management: {name}...")
    engine = Phase6Engine()
    
    # Base Config (Candidate B rules but allowing more frequency)
    config = {
        'entry_type': 1, 'timeframe': 'm3', 'fractal_n': 8,
        'start_hour': '08:30', 'end_hour': '12:00', # Expanded window
        'tp_val': 1.5, 'be_r': None, 'sl_type': 'sweep',
        'sl_plus_pips': 0.5, 'news_block_mins': 30,
        'one_trade_per_day': False, # Critical for management frequency
        'first_sweep_only': False,   # Critical for management frequency
...
def run_phase5():
    print("Phase 5: Management Frequency Laboratory")
    results = []
    
    # 1. Baseline 1.5R
    results.append(run_management_test("TP_1.5R", {"tp_val": 1.5}))
    
    # 2. Faster Exit 1.0R
    results.append(run_management_test("TP_1.0R", {"tp_val": 1.0}))
    
    # 3. BE Impact
    results.append(run_management_test("TP_1.5R_BE_0.8R", {"tp_val": 1.5, "be_r": 0.8}))

    out_dir = Path(r"C:\Users\alera\Desktop\Bot\BOT DE TRADING ultimo\BOT_V2_DAYTIME_LAB\outputs\phase9_frequency_expansion\management_frequency")
    out_dir.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(results).to_csv(out_dir / "management_frequency_results.csv", index=False)
    print("Management Laboratory Complete.")
