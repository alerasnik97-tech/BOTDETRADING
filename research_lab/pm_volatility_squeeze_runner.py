...
RESULTS_DIR = Path("results") / "pm_volatility_squeeze_retest_m5"
PAIR = "EURUSD"
TIMEFRAME = "M5"
STRATEGY_MODULES = [pm_volatility_squeeze_retest_m5]
...
def build_output_root() -> Path:
    ...
    root = RESULTS_DIR / f"{timestamp}_pm_volatility_squeeze_retest_m5"
...
def schedule_used() -> dict[str, str]:
    return {"entry_start": "11:00", "entry_end": "16:30", "force_close": "16:30"}
...
def evaluate_period(
    *,
    ...
    summary, trades_export, monthly_stats, yearly_stats, equity_export = summarize_result(
        ...
        break_even_setting=params.get("be_at_r"),
    )
    return {
...
def selection_score(summary: dict[str, Any]) -> float:
    ...
    score -= float(summary["max_drawdown_pct"]) * 2.8
...
def main() -> None:
    ...
    news_result = require_operational_news(PAIR, news_config, context="pm_volatility_squeeze_runner")
    ...
    scorecard = pd.DataFrame(strategy_rows)
    scorecard.to_csv(output_root / "pm_safe_vse_scorecard.csv", index=False)
        ...
        json.dumps(
            {
                ...
                "timeframe": TIMEFRAME,
                "periods": PERIODS,
                "news_file": str(PM_SAFE_NEWS_FILE),
