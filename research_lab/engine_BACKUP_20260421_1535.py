    ...
    for i in range(strategy_module.WARMUP_BARS, len(frame)):
        ...
        if position is not None:
            ...
            if forced_exit_pre_news and force_flat_mask[i]:
                if precision_enabled:
                    news_kill_open_price = float(bid_m15_open[i] if position.direction == "long" else ask_m15_open[i])
                else:
                    news_kill_open_price = float(open_[i])
                exit_reason, exit_signal_price, intrabar_ambiguity_flag, gap_exit_type = resolve_intrabar_exit(
                    direction=position.direction,
                    open_price=news_kill_open_price,
                    low_price=news_kill_open_price,
                    high_price=news_kill_open_price,
                ...
                if exit_reason is None:
                    ...
                    exit_signal_price = news_kill_open_price
                    exit_ts_utc = bar_open_utc[i]
            ...
            if exit_reason is not None and exit_signal_price is not None:
                ...
                blocked_by_news = bool(exit_reason == "news_fortress_kill")
                blocking_event_name = ""
                blocking_event_time_ny = ""
                blocking_rule_used = ""
                if blocked_by_news:
                    blocking_event_name = str(news_details["force_flat_event_name"].iat[i] or news_details["blocking_event_name"].iat[i])
                    blocking_event_time_ny = str(news_details["force_flat_event_time_ny"].iat[i] or news_details["blocking_event_time_ny"].iat[i])
                    blocking_rule_used = str(news_details["force_flat_rule_used"].iat[i] or news_details["blocking_rule_used"].iat[i])
                if precision_enabled:
        ...
        if position is None and pending_signal is None and i < len(frame) - 1:
            if i <= cooldown_until_index or not np.isfinite(range_atr[i]) or range_atr[i] > engine_config.shock_candle_atr_max:
                ...
                if signal is not None:
                    effective_signal_price = float(signal.get("limit_price", close[i])) if signal.get("entry_mode") == "limit" else float(close[i])
