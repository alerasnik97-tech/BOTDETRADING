...
def signal(frame: pd.DataFrame, i: int, params: dict) -> dict | None:
    ...
    # Contexto macroscópico (H1 pre-calculado en data_loader)
    h1_ema50_i = float(frame["h1_ema50"].iat[i])
    h1_ema200_i = float(frame["h1_ema200"].iat[i])
    h1_slope_i = float(frame["h1_ema200_slope_5"].iat[i])
    
    macro_bullish = (h1_ema50_i > h1_ema200_i) and (h1_slope_i > 0)
    macro_bearish = (h1_ema50_i < h1_ema200_i) and (h1_slope_i < 0)
    
    if not (macro_bullish or macro_bearish):
        return None # No hay alineación macro HTF direccional, abortamos prematuramente
    ...
    for j in range(i - expiry, i):
        ...
        # Breakout LONG respect to M5 EMA + HTF Bulls
        if macro_bullish and close_j > ema_val_j and float(frame["close"].iat[j-1]) <= res_R and close_j > res_R:
        ...
        # Breakout SHORT respect to M5 EMA + HTF Bears
        if macro_bearish and close_j < ema_val_j and float(frame["close"].iat[j-1]) >= sup_S and close_j < sup_S:
