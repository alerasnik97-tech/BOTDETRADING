#!/usr/bin/env python
"""
Laboratorio de bloques candidatos por par.

Optimiza una familia de estrategia en una ventana de diseno y luego
ejecuta fuera de muestra la mejor configuracion encontrada para cada par.
"""
...
def build_run_config(
    ...
    return bt.RunConfig(
        ...
        shock_guard_enabled=not disable_shock_guard,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Laboratorio de bloques candidatos por par.")
    ...
    parser.add_argument("--report-dir", default="reports_candidate_block_lab")
    ...
    for pair in args.pairs:
        ...
        print(f"\n[candidate-lab] Optimizando {args.strategy_family} en {pair}")
        base_params = bt.StrategyParameters(strategy_family=args.strategy_family)
        design_run = build_run_config(
            ...
            args.disable_shock_guard,
        )
        best_result, ranking = bt.run_grid_search(
            run_config=design_run,
        ...
        rows.append(summarize_row(pair, "design", best_result))
        ...
        oos_result = bt.run_backtest(oos_run, oos_params, broker, news_events=oos_news)
    ...
    summary.to_csv(output_root / "candidate_lab_summary.csv", index=False)
    ranking.to_csv(output_root / "candidate_lab_ranking.csv", index=False)
    (output_root / "candidate_lab_summary.json").write_text(
        json.dumps(summary.to_dict(orient="records"), indent=2),
        encoding="utf-8",
    )

    print("\n=== RESUMEN CANDIDATE LAB ===")
