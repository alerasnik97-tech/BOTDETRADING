...
def get_news_events(target_date):
    ...
    try:
        ...
        # Call the actual news filter
        settings = NewsConfig(file_path=os.path.join(ROOT, "research_lab", "data", "news", "news_events.csv"))
...
def process_day(target_date):
    ...
    if levels is None:
        ...
        }, ['session_date','sweeps_detected','sweeps_blocked_news','sweeps_blocked_daily_limit','sweeps_no_scbi','sweeps_data_issue','trades_paper','result','pnl_r','cumulative_n','cumulative_pf','cumulative_exp','cumulative_dd','incidents','notes'])
        sys.exit(0)
