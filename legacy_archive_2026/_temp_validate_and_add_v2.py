...
# Validacion
print("=" * 60)
print("VALIDACION DE FECHAS RECIBIDAS")
print("=" * 60)
...
for year, cpi_list, ppi_list in [
    ...
    for d in cpi_list:
        if not validate_date_format(d):
            validation_errors.append(f"CPI {year}: formato invalido {d}")
        all_dates.append((d, "CPI", year))
    
    # Validar PPI
    for d in ppi_list:
        if not validate_date_format(d):
            validation_errors.append(f"PPI {year}: formato invalido {d}")
...
# Check duplicados globales (mismo dia + mismo tipo)
...
if validation_errors:
    print(f"\n[X] ERRORES DETECTADOS:")
    for e in validation_errors:
        print(f"  - {e}")
else:
    print(f"\n[OK] Todas las fechas validas. Sin duplicados. Sin errores.")
