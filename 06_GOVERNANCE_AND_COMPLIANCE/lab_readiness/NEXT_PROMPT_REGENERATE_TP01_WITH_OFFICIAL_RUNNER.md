...
Actuá como Claude Opus 4.7 Max en modo **Institutional Quant Backtest Operator + Metric/Cost Reconciliation Gatekeeper** senior, backtest safety engineer y quant lab auditor.
...
Tests acumulados: 83/83 verdes (runner 27, cost 11, recon 19, engine 17, stop_entry 3, preflight 6).
...
el runner oficial, con 3 cost profiles reales y reconciliation gate obligatorio.

============================================================
PRECONDICIONES DURAS
============================================================

1. Usar SOLO `research_lab.runners.formal_train_runner`. PROHIBIDO usar
   `scratch/formal_run_tp01.py` (git-ignored, no sellable).
2. Runner debe correr con `FormalRunRequest.execute=True` SOLO tras preflight OK.
3. Si preflight o gate fallan → ABORTAR (`BLOCKED_PREFLIGHT` / `BLOCKED_RECONCILIATION_GATE`),
   NO sellar, NO commitear outputs pesados.
4. Branches de fix/infra disponibles por decisión del owner (NO mergear vos a main).
...
1. Precheck git + procesos (sin backtest activo). Crear branch
   `research/tp01-regen-official-runner-20260517` desde la cadena de fixes vigente.
2. Correr suite completa sin backtest:
   ...
   Falla → `BLOCKED_TEST_FAILURE`.
3. `preflight()` del runner para TP-01 (dry-run). Verificar 3 profiles reales,
   output bajo `…/BOT_V2_DAYTIME_LAB/reports/…`, manifest sin duplicados.
4. Ejecutar `run_single_strategy_formal_train_only(execute=True)` para TP-01,
   train-only 2015–2024, M1, profiles base/conservative/stress.
5. **Gate obligatorio**: `reconcile_all` (trades+equity+summary+profiles) ⇒ CERO
   violaciones. Verificar: `ending_equity ≈ 100000 + Σ pnl_usd`;
   `stop_loss⇒loss`/`take_profit⇒win`; `drawdown_pct` no todo 0;
   PF/expectancy/total_return coherentes; cada summary self-report su profile;
   base<conservative<stress en costo. Si falla → NO sellar.
6. Heavy outputs SOLO en `local_outputs_do_not_commit/` (NO commit).
...
8. Entregar a auditoría externa (protocolo v3) antes de cualquier conclusión.
...
- Stage explícito: solo reportes livianos + (si aplica) docs. NO data/heavy/scratch/ZIP/root.
  NO git add .
