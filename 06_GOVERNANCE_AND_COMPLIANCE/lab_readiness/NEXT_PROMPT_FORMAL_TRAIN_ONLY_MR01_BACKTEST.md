...
`TP01_METRIC_FIX_DONE`, incluyendo el **gate de reconciliación ledger↔dossier** en verde.

Motivo: la auditoría externa v2
(`TP01_FORMAL_DOSSIER_EXTERNAL_AUDIT_REPORT.md`) probó 4 defectos en la capa
**compartida** de métricas/costos del engine (equity desacoplada del ledger;
inversión de signo en ~49% de trades; `summary.json` auto-contradictorio;
cost profiles mislabeled/duplicados — sólo 2 corridas reales, conservative inexistente,
stress en conservative_mode). Correr MR-01 sobre el harness actual produciría un
dossier con **corrupción idéntica**. NO ejecutar MR-01 hasta levantar el gate.
...
- Entregar a auditoría externa (mismo protocolo v2 que TP-01) antes de cualquier conclusión.
