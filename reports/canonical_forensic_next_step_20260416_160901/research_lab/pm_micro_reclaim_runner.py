...
SCHEDULE_USED = {"entry_start": "11:30", "entry_end": "16:00", "force_close": "16:00"}
...
def progression_gate_from_row(row: pd.Series) -> bool:
    return (
        int(row["dev_total_trades"]) >= 12
        and float(row["dev_avg_trades_per_month"]) >= 0.20
        and float(row["dev_profit_factor"]) > 1.05
        and float(row["dev_expectancy_r"]) > 0.0
    )


def build_recommendation(selected_row: pd.Series, progression_candidates: int) -> str:
    lines = [
        "# Recomendacion final",
        "",
        ...
        "",
        "Decision rule:",
        "- Seleccion por score de desarrollo solamente.",
    ...
    if progression_candidates <= 0:
        lines.append("- Ninguna combinacion pasa el gate de progresion en development. La linea queda solo como prototipo diagnostico.")
    elif float(selected_row["val_profit_factor"]) > 1.0 and float(selected_row["hold_profit_factor"]) > 1.0:
        lines.append("- Hay senal suficiente para seguir auditando esta linea.")
    else:
        lines.append("- La linea queda implementada y auditada, pero todavia no demuestra edge defendible.")
    return "\n".join(lines)
...
def main() -> None:
        ...
        ranking_rows.append(
            {
                ...
                "progression_candidate_dev": False,
    ...
    ranking["progression_candidate_dev"] = ranking.apply(progression_gate_from_row, axis=1)
    selected = ranking.iloc[0]
    selected_params = json.loads(str(selected["parameter_set_used"]))
    progression_candidates = int(ranking["progression_candidate_dev"].sum())
    ...
    export_strategy_bundle(
        ...
        extra_json={
            ...
            "selection_metadata.json": {
                "progression_candidates_dev": progression_candidates,
                "selected_combo_id": int(selected["combo_id"]),
                "selected_combo_is_progression_candidate": bool(selected["progression_candidate_dev"]),
            },
        },
    )
    ...
    (output_root / "recomendacion_final.md").write_text(build_recommendation(selected, progression_candidates), encoding="utf-8")
